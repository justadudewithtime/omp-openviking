 ...
 * viking_* tools before the first await, makes sure the OpenViking server is
 * running (starting it once across all omp processes when needed), then hands
 * the upstream factory a host that captures its later registrations.
 */
...
export default async function (pi: ExtensionAPI) {
  ...
  // Awaited before upstream registers its session_start handler, so the
  // health check inside it succeeds even when we had to spawn the server.
  // Bounded by OPENVIKING_AUTOSTART_TIMEOUT_MS; degrades, never throws.
  await ensureServer();
